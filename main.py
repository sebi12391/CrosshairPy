...
class Crosshair(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowTitle("Crosshair Overlay")  # Set the window title
        self.setWindowFlags(
            Qt.WindowStaysOnTopHint | Qt.FramelessWindowHint | Qt.WindowTransparentForInput
        )

        # Get the screen resolution using screeninfo
        monitor = get_monitors()[0]  # Assuming primary monitor is the first one
        ...
        if SETTINGS["RESIZE_IMAGE"]:
            pixmap = original_pixmap.scaled(SETTINGS["RESIZE_TO_WIDTH"], SETTINGS["RESIZE_TO_HEIGHT"],
    ...
    def updateCrosshairPosition(self):
        if SETTINGS["FOLLOW_MOUSE"]:
            ...
            crosshair_x = cursor_x - self.crosshair_label.width() // 2
            crosshair_y = cursor_y - self.crosshair_label.height() // 2
        else:
            # Display the crosshair in the center of the screen
            crosshair_x = (self.width() - self.crosshair_label.width()) // 2
            crosshair_y = (self.height() - self.crosshair_label.height()) // 2
